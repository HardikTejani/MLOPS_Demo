...
os.system(f"apt-get -y update")
os.system(f"apt-get -y install curl")
os.system(f"apt-get -y install apt-transport-https ca-certificates")
os.system(f"curl https://packages.microsoft.com/keys/microsoft.asc | apt-key add -")
os.system(f"curl https://packages.microsoft.com/config/ubuntu/20.04/prod.list > /etc/apt/sources.list.d/mssql-release.list")
os.system(f"apt-get -y update")
os.system(f"ACCEPT_EULA=Y apt-get install -y msodbcsql17")
os.system(f"apt-get -y install unixodbc-dev")
...
class IRISClassification():
    ...
    def create_pipeline(self):
        ...
        SQL_Query = pd.read_sql_query("SELECT SepalLengthCm,SepalWidthCm,PetalLengthCm,PetalWidthCm,Species FROM IRISData" , conn) 
        final_df = pd.DataFrame(SQL_Query, columns=['SepalLengthCm','SepalWidthCm','PetalLengthCm','PetalWidthCm','Species'])
    ...
    def create_outputs(self, y_true, y_pred, X_test, name):
        ...
        #final_df.insert(0, 'DateandTime', pd.to_datetime('now').replace(microsecond=0))
        ...
        createOutputsql = "IF Object_Id('dbo.IRISOutput', 'U') IS NOT NULL \
        BEGIN \
        PRINT 'Output Table exist'\
        END \
        ELSE BEGIN \
        CREATE TABLE dbo.IRISOutput \
        ...
        PredictedSpecies INT NOT NULL); PRINT 'IRISOutput table created successfully.' END"

        with pyodbc.connect('DRIVER='+driver+';SERVER='+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD='+ password) as conn:
            with conn.cursor() as cursor:
                cursor.execute(createOutputsql)
                for index, row in final_df.iterrows():
                    cursor.execute("INSERT INTO dbo.IRISOutput (SepalLengthCm,SepalWidthCm,PetalLengthCm,PetalWidthCm,Species,PredictedSpecies) VALUES (?,?,?,?,?,?)", row['SepalLengthCm'], row['SepalWidthCm'], row['PetalLengthCm'], row['PetalWidthCm'], row['Actual Species'], row['Predicted Species'])
                cursor.commit()
        print("Output DF Info",final_df.info())
        print("Output DF Head",final_df.head())
        print("Output DF Head",final_df.tail())
        run=Run.get_context()
